...
export function reviewValidate(input: ValidateInput): ValidationResult {
  ...
  if (!existsSync(inputDir)) {
    const result: ValidationResult = {
      ...
      finalDecision: null,
      sourceArtifacts: [],
      errors: [{ code: "MISSING_RUN", message: `Run not found: ${runId}` }],
      warnings: [],
    };
  ...
  if (!existsSync(manifestPath)) {
    errors.push({
      ...
      message: "input-manifest.json not found",
  ...
  if (!existsSync(issueLedgerPath)) {
    errors.push({
      code: "MISSING_LEDGER",
      message: "issue-ledger.json not found",
      artifactPath: issueLedgerPath,
    });
  } else {
  ...
  if (!existsSync(coverageLedgerPath)) {
    errors.push({
      code: "MISSING_LEDGER",
      message: "coverage-ledger.json not found",
      artifactPath: coverageLedgerPath,
    });
  } else {
  ...
  if (!existsSync(synthesisPath)) {
    errors.push({
      ...
      message: "synthesis.json not found",
    ...
    if (issueLedger?.sourceArtifacts) {
      if (existsSync(brPath)) {
        ...
        if (issueLedger.sourceArtifacts.behaviorReviewHash !== brHash) {
          errors.push({
            ...
            message: "issue-ledger.behaviorReviewHash mismatch with current behavior-review.json",
          });
        }
      }
      if (existsSync(trPath)) {
        ...
        if (issueLedger.sourceArtifacts.testReviewHash !== trHash) {
          errors.push({
            code: "LEDGER_HASH_MISMATCH",
            message: "issue-ledger.testReviewHash mismatch with current test-review.json",
          });
        }
      }
      if (existsSync(eaPath)) {
        ...
        if (issueLedger.sourceArtifacts.evidenceAuditHash !== eaHash) {
          errors.push({
            code: "LEDGER_HASH_MISMATCH",
            message: "issue-ledger.evidenceAuditHash mismatch with current evidence-audit.json",
          });
        }
      }
    }
    ...
    if (coverageLedger?.sourceArtifacts) {
      if (existsSync(cmPath)) {
        ...
        if (coverageLedger.sourceArtifacts.changeMapHash !== cmHash) {
          errors.push({
            code: "LEDGER_HASH_MISMATCH",
            message: "coverage-ledger.changeMapHash mismatch with current change-map.json",
          });
        }
      }
      if (existsSync(brPath)) {
        ...
        if (coverageLedger.sourceArtifacts.behaviorReviewHash !== brHash) {
          errors.push({
            code: "LEDGER_HASH_MISMATCH",
            message:
              "coverage-ledger.behaviorReviewHash mismatch with current behavior-review.json",
          });
        }
      }
      if (existsSync(trPath)) {
        ...
        if (coverageLedger.sourceArtifacts.testReviewHash !== trHash) {
          errors.push({
            code: "LEDGER_HASH_MISMATCH",
            message: "coverage-ledger.testReviewHash mismatch with current test-review.json",
          });
        }
      }
      if (
        existsSync(verificationLedgerPath) &&
        coverageLedger.sourceArtifacts.verificationLedgerHash
      ) {
        ...
        if (coverageLedger.sourceArtifacts.verificationLedgerHash !== vlHash) {
          errors.push({
            code: "LEDGER_HASH_MISMATCH",
            message:
              "coverage-ledger.verificationLedgerHash mismatch with current verification-ledger.json",
          });
        }
      }
    }
  ...
  if (existsSync(synthesisPath) && existsSync(issueLedgerPath) && existsSync(coverageLedgerPath)) {
    ...
    if (!synthesis?.sourceArtifacts) {
      errors.push({
        code: "MISSING_SYNTHESIS_ARTIFACTS",
        message: "synthesis.json missing sourceArtifacts — hash chain cannot be verified",
      });
    } else {
      if (synthesis.sourceArtifacts.issueLedgerHash !== sha256(issueLedgerContent)) {
        errors.push({
          code: "SYNTHESIS_HASH_MISMATCH",
          message: "synthesis.issueLedgerHash mismatch with current issue-ledger.json",
        });
      }
      if (synthesis.sourceArtifacts.coverageLedgerHash !== sha256(coverageLedgerContent)) {
        errors.push({
          code: "SYNTHESIS_HASH_MISMATCH",
          message: "synthesis.coverageLedgerHash mismatch with current coverage-ledger.json",
        });
      }
      if (existsSync(verificationLedgerPath) && synthesis.sourceArtifacts.verificationLedgerHash) {
        ...
        if (synthesis.sourceArtifacts.verificationLedgerHash !== vlHash) {
          errors.push({
            code: "SYNTHESIS_HASH_MISMATCH",
            message:
              "synthesis.verificationLedgerHash mismatch with current verification-ledger.json",
          });
        }
      }
    }
    ...
    if (synthesis?.recommendation) {
      const hasBlocking =
        issueLedgerForDecision?.issues?.some((i: any) => i.candidateImpact === "merge_blocking") ??
        false;
      const verificationLedger = existsSync(verificationLedgerPath)
        ? (readJsonSafe(verificationLedgerPath) as any)
        : null;
      const hasFailedVerification =
        verificationLedger?.commands?.some((c: any) => c.status === "failed") ?? false;

      if (hasBlocking && synthesis.recommendation === "ready_to_merge") {
        errors.push({
          code: "DECISION_CONFLICT",
          message: "Synthesis recommends ready_to_merge but merge_blocking issues exist",
        });
      }
      if (hasFailedVerification && synthesis.recommendation === "ready_to_merge") {
        errors.push({
          code: "DECISION_CONFLICT",
          message: "Synthesis recommends ready_to_merge but verification has failed commands",
        });
      }
    }
  }
  ...
  const hasIntegrityErrors = errors.some(
    (e) =>
      e.code.includes("HASH_MISMATCH") ||
      e.code === "HEAD_CHANGED" ||
      e.code === "WORKSPACE_DIRTY" ||
      e.code === "DECISION_CONFLICT" ||
      e.code === "MISSING_SYNTHESIS_ARTIFACTS",
  );
  const hasMissingErrors = errors.some(
    (e) => e.code.startsWith("MISSING_") || e.code === "INVALID_MANIFEST",
  );
